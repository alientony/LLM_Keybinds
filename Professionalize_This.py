...
def standardize_text(prompt_text):
    ...
    final_text = '\n'.join(final_lines)
    Final_prompt = f"""
Refine the following text to enhance its clarity, elegance, and overall impact, focusing specifically on the language used, sentence structure, and the flow of ideas. Aim to make the message more engaging and compelling, ensuring it remains concise and to the point. The revision should preserve the original intent and key information but present it in a more polished, professional manner. Avoid introducing new concepts or steps not present in the original text; instead, concentrate on refining the existing message for a more sophisticated audience. Here's the text needing refinement:

"{final_text}"

The objective is to enhance the text's presentation and readability while keeping the essence and factual content intact. Please adjust the tone to align with a professional and sophisticated audience, ensuring the revised text is both accessible and appealing.
...
"""
    ...
    response = llm(
        Final_prompt,
        max_tokens=600,
        stop=["\n"],  # Adjust the stop token as needed
    )
    if response.get('choices'):
        generated_content = response['choices'][0].get('text', '')  # Extract the generated text
...
pyautogui.typewrite("""Type something here, press the hotkey, and the LLM will modify it and paste it in place of ur text.""", 0.002)
keyboard.wait('esc')
